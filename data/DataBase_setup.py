...
class Position(Basic):
    ...
    def info(self):
        return f"staff_num: {self.type} -- standard wage: {self.wage}{'; description: ' + self.name if self.name is not None else ''}"
...
class Award(Basic):
    ...
    def info(self):
        return f"{self.type} sum:{self.sum}{'; description: ' + self.name if self.name is not None or self.name != '' else ''}"
